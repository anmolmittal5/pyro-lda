...
if __name__ == '__main__':
    seed = 0
    df = pd.read_csv('/pyro-lda/test_data/test_data.csv')
    ...
    # Dump model weights/ beta as a pickle file    
    pickle.dump(beta, open('/pyro-lda/results/test_model.pkl', 'wb'))
    
    result_df = pd.DataFrame(columns=['top_words', 'coherence'])
    result_df['top_words'] = top_words
    result_df['coherence'] = topic_coherence
    result_df.to_csv('/pyro-lda/results/test_model_results.csv', index=False)
